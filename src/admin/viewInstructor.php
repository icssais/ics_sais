...
<div class="section_content" id="viewUser_section">
	<div class="section_label" id="view_instructor">
		<label>Instructor</label>
	</div>
	<div id="viewInstructor">
		<table style="margin-left:50px; text-align:center;" class='view_table'>
			...
			<?php
				...
				$query="SELECT * from user where role='Instructor'";
				$found=mysql_query($query, $conn);

				while($row=mysql_fetch_array($found)){
					echo "<tr>";?>
						<td><a class='simple_submit' href="index.php?action=viewInstructorProfile&username=<?php echo $row['username'] ?>"><?php echo $row['username'];?></a></td>
					<?php echo"
